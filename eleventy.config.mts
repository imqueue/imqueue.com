...
/** The two editions this repo builds, and the only keys the tables below carry. */
type Edition = "org" | "com";

const EDITION: Edition = process.env.EDITION?.toLowerCase() === "com" ? "com" : "org";
...
const ASSETS = buildAssetManifest(import.meta.dirname, EDITION);

// The data extension below has to resolve a module SYNCHRONOUSLY — Eleventy calls
// the parser and uses what it returns — and `import()` is a promise. This is the
// ESM way to keep `require`, and it is the only place in this file that needs it.
const nodeRequire = createRequire(import.meta.url);

export default function (eleventyConfig: UserConfig) {
  // ---- heading ids ---------------------------------------------------------
  // The rule, and the long explanation of why it is not markdown-it-anchor's
  // default, moved to scripts/lib/md-slug.ts — the search index deep-links to
  ...
  // ---- TypeScript templates and data files ---------------------------------
  //
  // Node strips types from a `.ts` file at load, so a `*.11ty.ts` template and a
  // `*.11tydata.ts` data file need no build step — but Eleventy still has to be
  // told the extensions exist, because it discovers both by suffix.
  //
  // `key: "11ty.js"` on the extension reuses Eleventy's OWN JavaScript template
  // engine rather than defining a new one: the file exports the same `data` and
  // `render` members it always did, and everything downstream — pagination,
  // permalinks, the data cascade — behaves identically because it IS the same
  // engine. Registering a fresh engine would mean reimplementing all of that.
  //
  // `read: false` on the data extension is what makes the parser receive a PATH
  // rather than the file's contents: these are modules to import, not text to
  // parse. `import()` is cached by specifier, which is also what keeps a watch
  // rebuild from re-evaluating an unchanged data file.
  eleventyConfig.addTemplateFormats(["11ty.ts"]);
  eleventyConfig.addExtension("11ty.ts", { key: "11ty.js" });
  //
  // The `.default` unwrap is required, not cosmetic. `require()` of an ES module
  // returns the NAMESPACE — `{ __esModule: true, default: … }` — so without it
  // Eleventy receives that wrapper as the data itself. The failure is quiet and
  // confusing: every field reads `undefined`, and the first symptom here was two
  // sitemap buckets both rendering to `sitemap-.xml` because the bucket name was
  // an empty interpolation rather than an error.
  eleventyConfig.addDataExtension("ts", {
    read: false,
    parser: (file) => {
      const mod = nodeRequire(path.resolve(file)) as Record<string, unknown>;
      const value = mod && mod.__esModule && "default" in mod ? mod.default : mod;

      // Eleventy's BUILT-IN JavaScript data loader calls a function export and
      // uses what it returns; a custom data extension's parser does not, so this
      // has to. Half of src/_data/ exports a function — sitemapBuckets, apiPackages
      // and atAGlance all compute from other data — and without the call Eleventy
      // received the function object itself and rejected it as "expected an Array
      // or an Object". A returned promise is fine: Eleventy awaits the parser.
  ...
  {
    // `yaml.load` answers `unknown`, which is right — a YAML file is whatever is
    // in it. Only the two fields this check is about are named.
    const site = yaml.load(fs.readFileSync("./src/_data/site.yml", "utf8")) as {
      definition?: unknown;
      definitionShort?: unknown;
    };
  ...
  const envId = (name: string): string | null =>
  ...
  // lost analytics.
  const ANALYTICS_DEFAULTS: Record<Edition, { ga4: string | null; clarity: string | null }> = {
  ...
  eleventyConfig.addFilter("isoDate", (value: Date | string | number) => {
  ...
  // with NO_MIRROR in lib/markdown-link.ts and the coverage filter in
  // scripts/check-sitemap.ts, which state the same rule for their own consumers.
  ...
  eleventyConfig.addCollection("llmsFull", (api) => {
    ...
    const take = (item: CollectionItem | undefined) => {
  ...
  /** One entry of src/_data/blogTopics.json, once it is known to be a real hub. */
  interface TopicMeta {
    label?: string;
    title: string;
    description?: string;
  ...
  // blogTopics.json is not a uniform record: it carries a `_comment` array of
  // strings beside the topics, and at least one topic has a `label` and nothing
  // else. So the file is read one entry at a time and the gate is the same one the
  // filter below always applied — a hub needs a `title` — rather than a shape
  // asserted over the whole file and then not checked.
  const topicMeta = (slug: string): TopicMeta | undefined => {
    const entry: unknown = (blogTopicMeta as Record<string, unknown>)[slug];

    return entry && typeof entry === "object" && typeof (entry as TopicMeta).title === "string"
      ? (entry as TopicMeta)
      : undefined;
  };

  eleventyConfig.addCollection("blogTopics", (api) => {
    const byTopic = new Map<string, CollectionItem[]>();

    for (const post of api
      .getFilteredByGlob("src/org/blog/posts/*.md")
      .filter((p) => !p.data.draft)
      .sort((a, b) => b.date.getTime() - a.date.getTime())) {
      for (const slug of (post.data.topics as string[] | undefined) || []) {
        const bucket = byTopic.get(slug);

        if (bucket) {
          bucket.push(post);
        } else {
          byTopic.set(slug, [post]);
        }
      }
    }

    return [...byTopic.entries()]
      .map(([slug, posts]) => ({ slug, posts, meta: topicMeta(slug) }))
      .filter((hub) => hub.posts.length >= MIN_TOPIC_POSTS && hub.meta)
      .map(({ slug, posts, meta: topic }) => ({
        slug,
        posts,
        label: topic?.label,
        title: topic?.title,
  ...
  eleventyConfig.addFilter("agentMarkdown", (content: unknown) => {
  ...
  const inlineToText = (s: unknown): string =>
    String(s)
  ...
  eleventyConfig.addFilter("faqPairs", (raw: unknown, wholePage = false) => {
  ...
  //     land on the element that can be opened. Both search-corpus.ts (which
  ...
  //     visible to a crawler and still satisfies check-jsonld.ts's assertion that
  ...
  eleventyConfig.addFilter("faqAccordion", (html: unknown) => {
    ...
    const blocks: { tag: string; attrs: string; inner: string; start: number; end: number }[] = [];
    ...
    blocks.forEach((block, i) => {
      const bodyEnd = blocks[i + 1]?.start ?? source.length;
  ...
  eleventyConfig.addFilter("definedTerms", (raw: unknown) => {
    const terms: { term: string; definition: string; section: string | null }[] = [];
  ...
  eleventyConfig.addFilter("bySection", (items: CollectionItem[] | undefined, prefix: string) =>
    (items || [])
  ...
  eleventyConfig.addFilter("latestModified", (posts: CollectionItem[] | undefined) => {
    // `modified` and `published` are both optional in the file — a page that has
    // never been edited carries only one of them — so this is `unknown` per field
    // rather than a shape claimed and then not checked. The `new Date(…)` below is
    // the check: anything unparseable becomes NaN and is filtered out.
    const dates: Record<string, { modified?: string; published?: string }> = pageDates;
    const stamps: unknown[] = Object.values(dates)
      .flatMap((entry) => [entry.modified, entry.published]);

    const times = stamps
      .concat((posts || []).map((p) => p.data.dateModified || p.date))
      .map((value) => (value ? new Date(value as string | Date).getTime() : NaN))
  ...
  eleventyConfig.addFilter("byAuthor", (posts: CollectionItem[] | undefined, slug: string) =>
    (posts || []).filter((p) => p.data.author === slug)
  );

  // Look up a single author record by slug from the authors data list.
  eleventyConfig.addFilter("authorBySlug", (authors: { slug: string }[] | undefined, slug: string) =>
  ...
  eleventyConfig.addFilter("related", (
    ...
    const scored = others
      .map((p) => ({
        p,
        score: ((p.data.topics as string[] | undefined) || []).filter((t) => want.has(t)).length,
      }))
  ...
  eleventyConfig.addFilter("postsByTopics", (
    posts: CollectionItem[] | undefined,
    topics: string[] | undefined,
    limit: number | undefined,
  ) => {
  ...
  // scripts/lib/asset-manifest.ts for the deploy bug this fixes, and
  ...
  eleventyConfig.addFilter("asset", function (url: string) {
  ...
  // the built markdown MIRRORS — see scripts/lib/search-corpus.ts for why that is
  ...
  // ---- browser scripts under --serve ---------------------------------------
  // src/_shared/js/*.ts is COMPILED (see emitBrowserScripts in
  // scripts/lib/asset-manifest.ts), and that compile runs once, here, at config
  // load. Everything else in src/ is a template Eleventy already watches, so
  // without these two lines editing consent.ts or site.ts during `npm run serve:*`
  // would rebuild the site around an unchanged .browser-js/ and serve the previous
  // version — the one failure mode a dev server must not have.
  //
  // The re-emit lands on the same hashed URL the manifest picked at load, which is
  // correct: the hash exists so a CHANGED asset gets a new URL in production, and
  // the dev server sends no immutable headers for anything to hold onto.
  eleventyConfig.addWatchTarget("src/_shared/js");
  eleventyConfig.on("eleventy.beforeWatch", () => {
    emitBrowserScripts(import.meta.dirname);
  });
