...
// keyed by URL, for scripts/lib/search-corpus.ts to fold into the search index.
//
...
//
// The search corpus is built from the markdown MIRRORS (see search-corpus.ts for why),
...
// already been bitten by duplicating (see the two `search-index.11ty.ts` headers). Asking
...
// by scripts/lib/api-summary.ts, and /api/search-index.json already carries them.

import type { EleventyData } from '../scripts/lib/eleventy.ts';

export default class SearchFrontmatter {
  data() {
    return {
  ...
  render(data: EleventyData) {
    const pages: Record<string, { d: string; k: string }> = {};
