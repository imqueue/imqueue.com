---
title: "AttributeNames enum · @imqueue/opentelemetry"
description: "Attribute keys this package sets on every span it creates."
apiCrumbs: [{"name":"API reference","url":"/api/"},{"name":"@imqueue/opentelemetry","url":"/api/opentelemetry/latest/"},{"name":"AttributeNames","url":"/api/opentelemetry/latest/opentelemetry.attributenames/"}]
---
...
`'server'` or `'client'` — see [TraceKind](/api/opentelemetry/latest/opentelemetry.tracekind/)<!-- -->. Duplicates the OpenTelemetry span kind as a plain attribute, for backends that only read attributes.
