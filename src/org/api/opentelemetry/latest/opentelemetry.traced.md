---
title: "traced() function · @imqueue/opentelemetry"
...
apiCrumbs: [{"name":"API reference","url":"/api/"},{"name":"@imqueue/opentelemetry","url":"/api/opentelemetry/latest/"},{"name":"traced","url":"/api/opentelemetry/latest/opentelemetry.traced/"}]
---
...
Partial&lt;[TracedOptions](/api/opentelemetry/latest/opentelemetry.tracedoptions/)<!-- -->&gt;


</td><td>

_(Optional)_ span kind, extra attributes and tracer name. `kind` defaults to [TraceKind.SERVER](/api/opentelemetry/latest/opentelemetry.tracekind/)<!-- -->; `tracerName` defaults to `'basic'`<!-- -->. Attributes given in `tags` are applied last, so they override the ones set automatically.
...
Every span it creates is named `method.call`<!-- -->; the decorated method is identified by the `resource.name` attribute (`ClassName.methodName`<!-- -->), not by the span name. The span is not made the active context, so spans created \*inside\* the method do not nest under it — for nesting, rely on [ImqueueInstrumentation](/api/opentelemetry/latest/opentelemetry.imqueueinstrumentation/)<!-- -->, which does establish context for RPC handlers.
