---
title: "signature() function · @imqueue/rpc v1.17.1 (archived)"
description: "Constructs and returns hash string for a given set of className, methodName and arguments."
apiCrumbs: [{"name":"API reference","url":"/api/"},{"name":"@imqueue/rpc","url":"/api/rpc/1.17.1/"},{"name":"signature","url":"/api/rpc/1.17.1/rpc.signature/"}]
noindex: true
latestUrl: "/api/rpc/latest/"
---

<!-- Do not edit this file. It is automatically generated by API Documenter. -->

# signature() function

Constructs and returns hash string for a given set of className, methodName and arguments.

**Signature:**

```typescript
export declare function signature(className: string, methodName: string | symbol, args: any[]): string;
```

## Parameters

<table><thead><tr><th>

Parameter


</th><th>

Type


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

className


</td><td>

string


</td><td>


</td></tr>
<tr><td>

methodName


</td><td>

string \| symbol


</td><td>


</td></tr>
<tr><td>

args


</td><td>

any\[\]


</td><td>


</td></tr>
</tbody></table>

**Returns:**

string

{<!-- -->string<!-- -->}

