---
title: "IMQLock.release() method · @imqueue/rpc v2.1.0 (archived)"
description: "Releases previously acquired lock for a given key"
apiCrumbs: [{"name":"API reference","url":"/api/"},{"name":"@imqueue/rpc","url":"/api/rpc/2.1.0/"},{"name":"IMQLock","url":"/api/rpc/2.1.0/rpc.imqlock/"},{"name":"release","url":"/api/rpc/2.1.0/rpc.imqlock.release/"}]
sitemap: false
noindex: true
latestUrl: "/api/rpc/latest/rpc.imqlock.release/"
---

<!-- Do not edit this file. It is automatically generated by API Documenter. -->

# IMQLock.release() method

Releases previously acquired lock for a given key

**Signature:**

```typescript
static release<T, E>(key: string, value?: T, err?: E): void;
```

## Parameters

<table><thead><tr><th>

Parameter


</th><th>

Type


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

key


</td><td>

string


</td><td>


</td></tr>
<tr><td>

value


</td><td>

T


</td><td>

_(Optional)_


</td></tr>
<tr><td>

err


</td><td>

E


</td><td>

_(Optional)_


</td></tr>
</tbody></table>

**Returns:**

void

