---
title: "ICacheConstructor interface · @imqueue/rpc"
description: "Constructor signature the registry uses to instantiate a cache adapter class."
apiCrumbs: [{"name":"API reference","url":"/api/"},{"name":"@imqueue/rpc","url":"/api/rpc/latest/"},{"name":"ICacheConstructor","url":"/api/rpc/latest/rpc.icacheconstructor/"}]
---

<!-- Do not edit this file. It is automatically generated by API Documenter. -->

# ICacheConstructor interface

Constructor signature the registry uses to instantiate a cache adapter class.

**Signature:**

```typescript
export interface ICacheConstructor 
```

## Remarks

The registry always calls it with no arguments. The optional `name` argument is a legacy affordance that no built-in adapter accepts and that the registry never supplies, so implementations must be constructible with zero arguments and set their own [ICache.name](/api/rpc/latest/rpc.icache.name/)<!-- -->.

## Methods

<table><thead><tr><th>

Method


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

[(new)(name)](/api/rpc/latest/rpc.icacheconstructor._new_/)


</td><td>

Constructs a cache adapter instance, ready to be initialized.


</td></tr>
</tbody></table>

