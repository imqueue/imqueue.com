---
title: "IMQService.publish() method · @imqueue/rpc"
description: "Broadcasts data on this service's pub/sub channel, delivering it to every client currently subscribed to this service."
apiCrumbs: [{"name":"API reference","url":"/api/"},{"name":"@imqueue/rpc","url":"/api/rpc/latest/"},{"name":"IMQService","url":"/api/rpc/latest/rpc.imqservice/"},{"name":"publish","url":"/api/rpc/latest/rpc.imqservice.publish/"}]
sitemap: false
---

<!-- Do not edit this file. It is automatically generated by API Documenter. -->

# IMQService.publish() method

Broadcasts data on this service's pub/sub channel, delivering it to every client currently subscribed to this service.

**Signature:**

```typescript
publish(data: JsonObject): Promise<void>;
```

## Parameters

<table><thead><tr><th>

Parameter


</th><th>

Type


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

data


</td><td>

JsonObject


</td><td>

JSON-serializable payload to broadcast


</td></tr>
</tbody></table>

**Returns:**

Promise&lt;void&gt;

## Exceptions

TypeError when the service has not been started, so the queue's writer is not connected

## Remarks

Fire-and-forget: plain pub/sub with no persistence, ordering guarantee or delivery confirmation, so subscribers that are offline lose the message.

