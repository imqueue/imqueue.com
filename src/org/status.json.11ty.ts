...
//
// The data is src/_data/packageStatus.json, written by scripts/gen-package-status.ts
...
//
// ORG ONLY, and for free: eleventy.config.mts:173 ignores src/org/** in the com
...
import type { StatusDoc } from '../../scripts/gen-package-status.ts';

export default class PackageStatusFeed {
  data() {
    return {
  ...
  render(data: EleventyData) {
    ...
    const abs = (u: string): string => (u && u.startsWith("/") ? site + u : u);
    // The shape comes from the generator that writes the file, not from a second
    // description of it here: src/_data/packageStatus.json is scripts/gen-package-status.ts's
    // output, and a field renamed there now fails to compile here.
    const status = data.packageStatus as StatusDoc;
