...
description: "Comparing @imqueue with the NestJS microservices module: application framework vs focused RPC transport, how the contract between caller and handler is typed, and when to use each or both."
...
To see `@imqueue`'s model in practice, the [**Tutorial**](/tutorial/) builds a multi-service app step by step with a GraphQL gateway in front. For the typing model specifically, see [type-safe service communication in TypeScript](/blog/type-safe-service-communication-typescript/), or [how @imqueue compares with tRPC](/blog/imqueue-vs-trpc/) if the boundary you care about is client–server. Shipping inside a closed-source product? See [commercial licensing & support](/license/).
