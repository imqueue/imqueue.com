...
| Addon packages | `--packages` | (none) — see [Package Catalog](/cli/package-catalog/) |
...
itself. [Package Catalog](/cli/package-catalog/#following-the-fleet) describes the
...
[Providers](/cli/providers/) for the details and tokens each one needs.
...
8. **Initialize git** locally (sets a local commit identity from the author/
   ...
   **remote**, **push**, and **tag** the initial version. By default the push
   ...
   [Configuration → Git transport](/cli/configuration/#git-transport-for-the-initial-push-https-vs-ssh).
...
`v4`). See [Custom Templates](/cli/custom-templates/) to build your own.
...
[Configuration](/cli/configuration/#per-service-overrides-imqrcjson).
