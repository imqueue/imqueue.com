...
// `with { type: "json" }` is not optional under ESM: Node refuses a bare JSON
// import outright (ERR_IMPORT_ATTRIBUTE_MISSING), which is what the CommonJS
// require() this replaced was hiding.
import versions from './apiVersions.json' with { type: 'json' };
...
const published: Record<string, { latest?: string } | undefined> = versions;
const latest = (pkg: string): string | null => published[pkg]?.latest || null;
