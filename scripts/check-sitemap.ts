#!/usr/bin/env node
// check-sitemap.ts — guards the sitemap against the ways it has silently broken.
//
//   node scripts/check-sitemap.ts
//
...
const locsIn = (xml: string): string[] =>
  [...xml.matchAll(/<loc>\s*([^<]+?)\s*<\/loc>/g)].map((m) => m[1] ?? '');

// Mirrors the `sitemap: false` rule in scripts/build-api-docs.ts: a per-symbol
...
function isApiMemberPage(url: string): boolean {
  const seg = url.replace(/\/$/, '').split('/').pop() ?? '';

  return url.includes('/api/') && seg.split('.').length > 2;
}

/** What check-sitemap needs to know about one built page. */
interface BuiltPage {
  noindex: boolean;
}

// Every built page, with whether it is indexable, keyed by site-absolute URL.
function builtPages(dir: string): Map<string, BuiltPage> {
...
for (const [edition, origin] of Object.entries(EDITIONS)) {
  ...
  // reopening. See src/com/mirrors/README.txt.
  {
    const unmirrored = [...pages.entries()]
      ...
      // this in step with API_MIRRORED / API_AUTHORED in eleventy.config.mts — the
      // first covers only the /latest/ trees, because the /api/ landing page is
      // mirrored by hand from src/org/mirrors/api.liquid rather than from its
      // template source, and the second covers /api/faq/, which is ordinary prose
      // and mirrored from its own markdown.
      ...
      .filter(([url]) => !url.startsWith('/api/')
        || /^\/api\/$|^\/api\/faq\/$|^\/api\/[^/]+\/latest\//.test(url))
