...
// somebody editing a build command in two dashboards. scripts/build-site.ts closes that:
...
function copyPeers() {
  ...
  for (const target of EDITIONS) {
    // Non-null by construction: `present.length < 2` returned above, and EDITIONS
    // has exactly two entries, so the other one is always there.
