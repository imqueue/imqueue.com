#!/usr/bin/env node
// gen-package-status.ts — what every published @imqueue package IS, right now, in a
// form something other than a browser can read.
//
//   node scripts/gen-package-status.ts            write src/_data/packageStatus.json
//   node scripts/gen-package-status.ts --check    exit 1 if the committed file is stale
//
...
// checked out. See scripts/lib/npm-releases.ts.
//
...
/** One package's row in the status feed. Every value is a fact from npm. */
export interface PackageFacts {
  name: string;
  scoped: string;
  version: string;
  license: string;
  /** null means "this package does not say", never "it says the framework floor". */
  node: string | null;
...
/** src/_data/packageStatus.json as a whole. */
...
/** The manifest fields this generator reads back off npm. */
interface Manifest {
  license?: string;
  engines?: { node?: string };
  deprecated?: unknown;
...
// Sourced from src/_data/atAGlance.ts — the same rows /intro/ renders and llms.txt
...
function frameworkFacts(): FrameworkFacts {
  ...
  const pick = (label: string): string => {
    ...
    if (!row) {
      throw new Error(
        `gen-package-status: src/_data/atAGlance.ts has no "${label}" row any more. ` +
...
function factsFor(pkg: StatusPackage): PackageFacts {
  ...
  // Non-null by construction: the guard above returned on an empty list.
  const version = versions[versions.length - 1] ?? '';
  const manifest = packageManifest(pkg.name, version) as Manifest;
  const times = releaseTimes(pkg.name);
  const day = (v: string | undefined): string | null =>
    (v && times[v] ? times[v]?.slice(0, 10) ?? null : null);
  ...
  return {
    ...
    // in src/_data/apiPackages.ts. Every package lives at github.com/imqueue/<name>.
...
// reach the mirror or llms-full.txt — check-llms.ts fails the build on a surviving
// comment, and that assertion stays honest.

const BLOCK = (name: string): { begin: string; end: string } => ({
...
function renderPage(doc: StatusDoc): string {
...
const built: StatusDoc =
  { generated: new Date().toISOString(), framework: frameworkFacts(), packages };
...
const claim = (doc: StatusDoc): string =>
  JSON.stringify({ framework: doc.framework, packages: doc.packages });
...
if (CHECK) {
  ...
  const committed = JSON.parse(fs.readFileSync(OUT, 'utf8')) as StatusDoc;
  ...
  if (claim(committed) !== claim(built)) {
    const before = new Map<string, PackageFacts>(committed.packages.map(p => [p.name, p]));
    ...
    for (const p of built.packages) {
      ...
      // Keyed off PackageFacts rather than a bare string list, so a field renamed
      // there and not here stops compiling instead of silently reporting nothing.
      const COMPARED: Array<keyof PackageFacts> =
        ['version', 'license', 'node', 'released', 'deprecated'];
