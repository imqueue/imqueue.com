...
const RENAMED_PAGES = new Map();

function recordRenames({ pkg, seg, renames }) {
  for (const { from, to, kind } of renames) {
    ...
    const slug = (name) => `${pkg}/${seg}/${pkg}.${name.toLowerCase()}`;
...
`);
  console.log(
    `Wrote lib/api-renames.js — ${rows.length} page(s) 301 from a stripped ` +
    'collision suffix',
  );
}
...
//
// Generated from the hand-maintained RENAMED_PACKAGES in lib/api-packages.js, NOT
...
// TypeDoc-era salvage in lib/api-redirects.js.
//
...
function writeLegacyPages() {
  ...
  const file = path.join(ROOT, 'lib', 'api-legacy-pages.js');
...
`);
  console.log(`Wrote lib/api-legacy-pages.js: ${entries.length} archived pages`);
}
...
function writeRenamedModule() {
  const body = RENAMED_PACKAGES
    ...
    .join('\n');
  const file = path.join(ROOT, 'lib', 'api-renamed.js');
  fs.mkdirSync(path.dirname(file), { recursive: true });
  fs.writeFileSync(file, `// GENERATED by scripts/build-api-docs.js — do not edit by hand.
//
...
`);
  console.log(`Wrote lib/api-renamed.js: ${RENAMED_PACKAGES.length} rename(s)`);
}
...
`);
  ...
  // _redirects (see lib/api-handler.js), so without one the request never reaches
