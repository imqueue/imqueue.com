#!/usr/bin/env node
// check-llms.ts — guards /llms.txt and /llms-full.txt against the ways they have
// silently broken, on both editions.
//
//   node scripts/check-llms.ts
//
...
/** One `## ` section, with its items and anything that is not one. */
interface LlmsSection {
  name: string;
  items: LlmsItem[];
  /** Lines inside the section that are not link items — none is the rule. */
  prose: string[];
  lines: string[];
}
...
// built separately. check-links.ts crawls both and owns those.
const EXTERNAL_OK = (url: string, origin: string): boolean =>
  /^https?:\/\//.test(url) && !url.startsWith(origin);

function parse(text: string): { intro: string; sections: LlmsSection[] } {
...
function resolves(dir: string, url: string, origin: string): boolean | null {
...
for (const [edition, origin] of Object.entries(EDITIONS)) {
  ...
  // engines exist.
  //
  // API member pages are excluded exactly as check-sitemap.ts excludes them: the
  ...
  const isGeneratedSymbolPage = (url: string): boolean =>
    /^\/api\/[^/]+\/[^/]+\/.+/.test(url);
  ...
  if (edition === 'org') {
    ...
    if (start < 0) {
      ...
      const toc: Array<{ title: string; url: string }> = [];
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (!/^Source: https:\/\//.test(line)) continue;
  ...
  const SURFACES: ReadonlyArray<readonly [string, string]> =
    [['llms.txt', text], ['llms-full.txt', full]];
