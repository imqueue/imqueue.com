#!/usr/bin/env node
// check-package-status.ts — guards src/_data/packageStatus.json, offline.
//
//   node scripts/check-package-status.ts
//
...
// `gen-package-status.ts --check`, which needs the network and is wired into a
// workflow instead. Exactly the split check-api-versions.ts already uses.
//
...
import { statusPackages } from './lib/api-packages.ts';
...
/** One row of the generated src/_data/packageStatus.json. */
interface StatusRow {
  name: string;
  version?: string;
  license?: string;
...
/** The framework-wide block, which is one flat record of strings. */
...
try {
  doc = JSON.parse(fs.readFileSync(FILE, 'utf8')) as StatusDoc;
} catch (e) {
  const why = e instanceof Error ? e.message : String(e);

  console.error(`  FAIL  src/_data/packageStatus.json is not valid JSON: ${why}`);
  process.exit(1);
}

// ---- coverage ---------------------------------------------------------------
// Against api-packages.ts rather than against a list repeated here. Adding a
...
if (extra.length) {
  fail(`packageStatus.json lists ${extra.join(', ')}, which api-packages.ts does not`);
}
if (!missing.length && !extra.length) {
  pass(`all ${expected.length} packages from api-packages.ts are present`);
}
...
// ---- the framework block -----------------------------------------------------
// Keyed off the interface rather than a bare string list, so a field renamed in
// FrameworkFacts and not here stops compiling instead of failing at runtime.
const FRAMEWORK_KEYS: Array<keyof FrameworkFacts> =
  ['license', 'licenseNote', 'commercial', 'node', 'redis'];
