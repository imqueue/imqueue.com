 ...
 *   node scripts/indexnow-ping.ts org --print-urls --live  # same, from the deployed site
 *
...
function readKey() {
    const yml = fs.readFileSync(
        path.join(import.meta.dirname, '..', 'src', '_data', 'site.yml'),
        'utf8',
    );
...
const isIndex = (xml: string): boolean => xml.includes('<sitemapindex');
...
function readLocal(edition: string, name: string): string {
    const file = path.join(import.meta.dirname, '..', `_site-${edition}`, name);
...
function urlsFromSitemap(edition: string): string[] {
...
async function urlsFromLiveSitemap(host: string): Promise<string[]> {
...
async function main() {
    ...
    if (!edition || !HOSTS[edition]) {
        console.error('Usage: node scripts/indexnow-ping.ts <com|org> [paths…] [--dry-run]');
        process.exit(1);
    }
