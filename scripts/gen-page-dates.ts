#!/usr/bin/env node
// gen-page-dates.ts — record real publication/modification dates for the
// hand-authored pages, keyed by source path, into src/_data/pageDates.json.
//
//   node scripts/gen-page-dates.ts [--check]
//
...
/** The two dates one page carries, both ISO-8601 in UTC. */
interface PageDates {
  published: string;
  modified: string;
}

/** src/_data/pageDates.json: repo-relative source path -> its dates. */
type DateIndex = Record<string, PageDates | undefined>;
...
const utcZ = (iso: string): string => iso.replace(/\+00:00$/, 'Z');
...
function datesFor(rel: string): PageDates | null {
...
if (process.argv.includes('--check')) {
  const committed: DateIndex =
    fs.existsSync(OUT) ? JSON.parse(fs.readFileSync(OUT, 'utf8')) as DateIndex : {};
...
if (process.argv.includes('--fix-modified')) {
  ...
  const committed = JSON.parse(fs.readFileSync(OUT, 'utf8')) as DateIndex;
  const moved: Array<{ key: string; from: string; to: string }> = [];

  for (const [key, entry] of Object.entries(committed)) {
    const fresh = dates[key];

    if (!entry || !fresh || fresh.modified === entry.modified) continue;
