#!/usr/bin/env node
// backfill-api-descriptions.js — add `description:` front matter to the already
// generated API reference pages under src/org/api/.
//
// scripts/build-api-docs.js now emits a per-symbol description, but rerunning it
...
// place with the same scripts/lib/api-summary.js the generator uses.
//
// Idempotent: pages that already carry a description are left alone unless
// --force is passed.
//
//   node scripts/backfill-api-descriptions.js [--dry-run] [--force]
...
for (const { pkg, version, file } of apiPages()) {
  ...
  const symbol = titleMatch
    ? titleMatch[1].split(' · ')[0]
