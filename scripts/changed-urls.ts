#!/usr/bin/env node
/*
 * changed-urls.ts — the site URLs a git diff actually touched.
 *
 *   node scripts/changed-urls.ts <edition> <git-range> [--prefix=/api/]
 *
 *   node scripts/changed-urls.ts org HEAD~1..HEAD
 *   node scripts/changed-urls.ts org HEAD~1..HEAD --prefix=/api/
 *
 * Prints one absolute URL per line, suitable for:
 *
 *   node scripts/indexnow-ping.ts org $(node scripts/changed-urls.ts org HEAD~1..HEAD)
 *
...
const HOSTS: Record<string, string | undefined> = { com: 'imqueue.com', org: 'imqueue.org' };

const [edition = '', range, ...rest] = process.argv.slice(2);

if (!edition || !HOSTS[edition] || !range) {
  console.error('usage: changed-urls.ts <org|com> <git-range> [--prefix=/api/]');
  process.exit(2);
}
...
function explicitPermalink(file: string): string | null {
...
// so expand one level — the same thing indexnow-ping.ts does, for the same reason.
