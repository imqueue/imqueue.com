#!/usr/bin/env node
// verify.js — run the checks and the e2e suite, and answer the only question a
// regression run is actually asking: DID I BREAK THIS?
//
//   node scripts/verify.js --save          record the current state as the baseline
//   node scripts/verify.js                 run again and diff against that baseline
//   node scripts/verify.js --scope=content  only the checks a copy edit can affect
//   node scripts/verify.js --no-build      reuse the existing _site-* (faster, riskier)
//
...
const checks = scope === 'full' ? allChecks : allChecks.filter((c) => TIERS[scope].includes(c));
...
function runChecks() {
  ...
  for (const name of checks) {
    ...
    results[name] = { ok, gist: ok ? '' : gist(result) };
    console.log(ok ? 'ok' : `FAIL  ${results[name].gist}`);
  }
...
function runE2eSuite() {
  ...
  if (!fs.existsSync(E2E_JSON)) {
    ...
    return { ran: false, failing: [], total: 0, exit: result.status };
  }

  const report = JSON.parse(fs.readFileSync(E2E_JSON, 'utf8'));
  const failing = [];
  let total = 0;
  ...
  return { ran: true, failing, total, exit: result.status };
}

// ---- comparing ------------------------------------------------------------

/** Every failing thing in a run, as one flat set of stable names. */
function failuresOf(state) {
  const names = Object.entries(state.checks)
    .filter(([, r]) => !r.ok)
    .map(([name]) => name);

  return new Set([...names, ...state.e2e.failing.map((t) => `e2e: ${t}`)]);
}
...
const state = {
  ...
  scope,
  checks: runChecks(),
  e2e: runE2e ? runE2eSuite() : { ran: false, failing: [], total: 0 },
};
