#!/usr/bin/env node
// check-redirects.ts — guards the two things that broke silently once.
//
...
//   2. The /api/ version mapping now lives in lib/api-redirects.ts instead. This
...
//
//   node scripts/check-redirects.ts
//
// Exits non-zero on any failure; wired into `npm test`.

import fs from 'node:fs';
import path from 'node:path';
import { RENAMED_PACKAGES as CONFIG } from './lib/api-packages.ts';


const ROOT = path.join(import.meta.dirname, '..');
const DYNAMIC_LIMIT = 100;
const FIXTURE = path.join(import.meta.dirname, 'fixtures', 'legacy-api-redirects.txt');
...
async function checkLegacyApiRules() {
  const { resolveApiRedirect } = await import('../lib/api-redirects.ts');
  ...
  const probes = ['', 'rpc.imq/', 'core.ts/nested/'];
  let checked = 0;
  ...
  const { API_VERSIONS } = await import('../lib/api-versions.ts');
  let kept = 0;

  for (const [pkg, plan] of Object.entries(API_VERSIONS)) {
    // API_VERSIONS is typed with an optional value on purpose — every lookup in
    // the Pages Function is on an untrusted path segment — but iterating its own
    // entries can only ever yield the ones that are there.
    if (!plan) continue;
...
async function checkRenamedPackages() {
  const { resolveApiRedirect, resolveRenamedPackage } =
    await import('../lib/api-redirects.ts');
  const { API_VERSIONS } = await import('../lib/api-versions.ts');
  const { RENAMED_PACKAGES: MAP } = await import('../lib/api-renamed.ts');
  ...
  if (JSON.stringify(generated) !== JSON.stringify(configured)) {
    fail(
      'lib/api-renamed.ts is out of step with RENAMED_PACKAGES in ' +
      `scripts/lib/api-packages.ts (generated: ${generated.join(', ') || 'none'}; ` +
      `configured: ${configured.join(', ') || 'none'}) — run npm run build-docs`,
    );
  } else {
    pass(`lib/api-renamed.ts matches the config (${generated.length} rename(s))`);
  }
  ...
  for (const [from, to] of MAP) {
    ...
    const mustBeMounted = path.join(ROOT, 'functions', 'api', from, '[[path]].ts');
    ...
    if (!fs.existsSync(mustBeMounted)) {
      fail(`functions/api/${from}/[[path]].ts is missing — /api/${from}/* would 404`);
    } else {
...
function pagesOf(pkg: string): string[] {
...
function existsOnDisk(pathname: string): boolean {
...
async function checkCoreReexports() {
  const { resolveCoreReexport } = await import('../lib/api-redirects.ts');
  const { CORE_REEXPORTS } = await import('../lib/api-crosslinks.ts');
...
async function checkRenamedPages() {
  const { resolveRenamedApiPage } = await import('../lib/api-redirects.ts');
  const { RENAMED_API_PAGES } = await import('../lib/api-renames.ts');
  const pageFor = (slug: string): string => path.join(ROOT, 'src', 'org', 'api', `${slug}.md`);
  let broken = 0;
...
async function checkLegacyTypedoc() {
  const { resolveLegacyTypedoc } = await import('../lib/api-redirects.ts');
  ...
  const { ARCHIVED_PAGES } = await import('../lib/api-legacy-pages.ts');
  let invented = 0;

  const GHOST_TREES: ReadonlyArray<readonly [string, string]> =
    [['rpc', '2.1.0'], ['core', '1.15.0']];

  for (const [pkg, seg] of GHOST_TREES) {
  ...
  if (missing.length || stale.length) {
    fail(
      `lib/api-legacy-pages.ts is out of step with the tree: ${missing.length} page(s) `
      + `missing, ${stale.length} stale. Re-run \`npm run build-docs\`.`,
    );
  } else {
    pass(`lib/api-legacy-pages.ts matches the tree (${ARCHIVED_PAGES.size} archived pages)`);
  }
