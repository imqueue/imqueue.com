#!/usr/bin/env node
// check-search-ranking.ts — asserts what the search ranker must return.
//
//   node scripts/check-search-ranking.ts
//
...
/** One scored hit, as the ranker returns it. */
interface Hit {
  score: number;
  /** Set when the hit came from the PEER edition's feed. */
  external?: boolean;
  record: { u: string; t: string; g?: number; p?: string };
}

/**
 * The ranker's mutable scratch space, as this check drives it.
 *
 * A local shape rather than a field on RankerEngine: `state` is the engine's own
 * internals, and this file is the only thing in the repo that WRITES to it — it
 * swaps a synthetic corpus in to isolate the element weights, then swaps the real
 * one back. See scripts/lib/ranker.ts for why the engine's type stays open.
 */
interface RankerState {
  t1: unknown;
  /**
   * The prepared tier-2 corpus. The fields named here are the ones this file
   * reads back out of it — the rest of what prepareSections() builds is the
   * engine's business.
   */
  t2: {
...
const state = ranker.state as unknown as RankerState;
...
state.t1 = ranker.prepare(JSON.parse(fs.readFileSync(path.join(OUT, 'search-index.json'), 'utf8')));
state.t2 = ranker.prepareSections(JSON.parse(fs.readFileSync(path.join(OUT, 'search-text.json'), 'utf8')));

const run = (query: string): Hit[] => ranker.search(ranker.parseQuery(query));

/**
 * Score per anchor slot, from whichever corpus is currently loaded.
 *
 * The three synthetic probes below each ran this same loop into their own object,
 * and each then read the slots back with a `|| 0` fallback. One function says the
 * rule once: an absent slot scored 0, because the probe document did not come back
 * at all — which is exactly what those fallbacks meant.
 */
function scoresFor(query: string): (slot: string) => number {
  const found: Record<string, number> = {};

  for (const hit of run(query)) {
    found[hit.record.u.split('#')[1] || 'page'] = hit.score;
  }

  return (slot) => found[slot] ?? 0;
}
...
for (const testCase of CASES) {
  ...
  if (top.record.u !== testCase.url) {
    fail(
      ...
      `        top 3: ${hits.slice(0, 3).map((h: Hit) => `${Math.round(h.score)} ${h.record.u}`).join(' | ')}`
    );
    continue;
  }
  if (testCase.keepsTop) {
    const { url, within } = testCase.keepsTop;
    const at = hits.findIndex((h: Hit) => h.record.u === url);
    ...
    if (at < 0 || at >= within) {
      fail(
        ...
        `        protects: ${testCase.protects}\n` +
        `        top ${within}: ${hits.slice(0, within).map((h: Hit) => `${Math.round(h.score)} ${h.record.u}`).join(' | ')}`
      );
      continue;
    }
...
const scoped = run('pkg:rpc lock').filter((hit: Hit) => hit.record.g === 1);
const foreign = scoped.filter((hit: Hit) => hit.record.p !== '@imqueue/rpc');
...
} else if ((bare[0]?.score ?? 0) >= (content[0]?.score ?? 0)) {
  fail(`"the" scored ${Math.round(bare[0]?.score ?? 0)}, not below "circuit breaker" at ${Math.round(content[0]?.score ?? 0)} — stopwords are not being de-weighted`);
} else {
  pass(`"the" is searchable (${bare.length} hits) and scores below a content query (${Math.round(bare[0]?.score ?? 0)} < ${Math.round(content[0]?.score ?? 0)})`);
}
...
// Kept so each synthetic probe can be swapped out and the real corpus restored.
// Typed off `state` rather than off `ranker.state`, whose type is deliberately
// open — see RankerState above.
const saved = state.t2;
...
state.t2 = ranker.prepareSections(ELEMENT_PROBE);
if (state.t2) state.t2.df = null;

const probe = scoresFor('widget');
...
const ordered = probe('a') > probe('b') && probe('b') > probe('c')
  && probe('a') > probe('d') && probe('d') > 0;

if (!ordered) {
  fail(
    'element weights are out of order — expected heading > emphasis > body, and heading > page-title-only, got ' +
    `heading ${Math.round(probe('a'))}, emphasis ${Math.round(probe('b'))}, ` +
    `body ${Math.round(probe('c'))}, page title ${Math.round(probe('d'))}`
  );
} else {
  pass(
...
state.t2 = ranker.prepareSections(DENSITY_PROBE);
if (state.t2) state.t2.df = null;

const density = scoresFor('widget');

state.t2 = saved;

if (!(density('dense') > density('sparse'))) {
  fail(`keyword density is not counted: dense ${Math.round(density('dense'))} vs sparse ${Math.round(density('sparse'))}`);
} else {
...
state.t2 = ranker.prepareSections(POSITION_PROBE);
if (state.t2) state.t2.df = null;

const position = scoresFor('what is imqueue');

state.t2 = saved;

if (!(position('x') > position('y'))) {
  fail(`word order is not scored: "what is imqueue" ${Math.round(position('x'))} vs "what imqueue is" ${Math.round(position('y'))}`);
...
state.t2 = ranker.prepareSections(HYPHEN_PROBE);
if (state.t2) state.t2.df = null;
...
for (const [base, inflected] of INFLECTED) {
  ...
  const at = b.findIndex((hit: Hit) => hit.record.u === a[0]?.record.u);
...
const IRREGULAR: ReadonlyArray<readonly [string, string]> =
  [['went', 'go'], ['gone', 'go'], ['mice', 'mouse'], ['indices', 'index']];
const lemmaMap = state.t2?.lemmas ?? {};
...
const sendHits = run('send').filter((hit: Hit) => hit.record.g === 1);
...
const KEYWORD_CASES: ReadonlyArray<readonly [string, string, string]> = [
...
for (const [query, target, was] of KEYWORD_CASES) {
  ...
  const at = hits.findIndex((hit: Hit) => hit.record.u.split('#')[0] === target);
...
const LICENSE_PHRASINGS: ReadonlyArray<readonly [string, number]> = [
  ['imqueue license', 1],
  ['licensing', 1],
...
for (const [query, worst] of LICENSE_PHRASINGS) {
  const hits = run(query);
  const at = hits.findIndex((hit: Hit) => hit.record.u.split('#')[0] === '/license/');
...
// origin (scripts/copy-peer-index.ts). Two properties matter, and they pull against each
...
if (!fs.existsSync(peerIndex)) {
  ...
  for (const probe of ORDER_PROBES) {
    ...
    const firstPeer = hits.findIndex((hit: Hit) => hit.external);
    const lastLocal = hits.reduce((at: number, hit: Hit, i: number) => (hit.external ? at : i), -1);
  ...
  const pricingPeers = ranker.search(ranker.parseQuery('pricing')).filter((hit: Hit) => hit.external);
  ...
  const withPeer = ranker.search(q).filter((hit: Hit) => hit.external);
...
// more of the same, and the page that answers the question was not in the 72 results.
{
  ...
  const at = hits.findIndex((hit: Hit) => hit.record.u === target);
  ...
  const gateway = hits.findIndex((hit: Hit) => /graphql-or-rest-gateway$/.test(hit.record.u));
...
// The words the -ly route must NOT touch, and the evidence is in scripts/lib/lemma.ts:
...
// not find it at all, while every other element in the ranker matched substrings.
{
  ...
  for (const [query, url] of cases) {
    ...
    const at = hits.findIndex((hit: Hit) => hit.record.u === url);
...
// #1 to #9 for the query "what is imqueue".
{
  ...
  const home = hits.findIndex((hit: Hit) => hit.record.u === '/');
...
// literal "---" mid-snippet.
{
  const glued = (state.t2?.sections ?? [])
    .filter((section) => /(^|\s)-{3,}(\s|$)/.test(section[3]));

  if (glued.length) {
    fail(
      `${glued.length} section(s) still carry a thematic break in their text, ` +
      `e.g. "${glued[0]?.[2] || (state.t2?.pages ?? [])[glued[0]?.[0] ?? 0]?.[1]}"`
    );
  } else {
...
// produce one, and that is a supported local state.
{
  ...
  if (!fs.existsSync(peerIndex) || !fs.existsSync(peerText)) {
    ...
    for (const [query, why] of COMMERCIAL) {
      const hits = ranker.search(ranker.parseQuery(query));
      const peer = hits.findIndex((hit: Hit) => hit.external);
    ...
    const local = ranker.search(ranker.parseQuery('redis queue')).findIndex((hit: Hit) => !hit.external);
