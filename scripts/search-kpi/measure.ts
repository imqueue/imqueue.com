#!/usr/bin/env node
// measure.ts — THE TRIPWIRE, not the KPI. `npm run kpi` (gold.ts) is the KPI.
//
//   node scripts/search-kpi/measure.ts                    the artificial set
//   node scripts/search-kpi/measure.ts --index DIR        measure a snapshot instead
//   node scripts/search-kpi/measure.ts --worst 40         list the worst misses
//   node scripts/search-kpi/measure.ts --json FILE        write the full result for diffing
//   node scripts/search-kpi/measure.ts --compare FILE     diff against an earlier --json run
//
...
import {
  load, evaluate, summarise, table, type ScoredResult, type Summary, type TestCase,
} from './lib/harness.ts';
...
/**
 * One case of the generated artificial set. `bucket` is the query SHAPE the
 * generator used (title-salient, body-salient, …), which is what the macro here
 * averages over.
 */
interface ArtificialCase extends TestCase {
  bucket?: string;
}

/** One row of the per-bucket table. */
interface MacroRow {
  key: string;
  n: number;
  accuracy: number;
...
/** The per-bucket table plus its equal-weight mean. */
...
/** One query's position, as stored in a --json run for later comparison. */
...
/** What a --json run writes for the artificial set. */
interface ArtificialReport {
  summary: Summary;
  macro: number;
  strict?: number;
  buckets: MacroRow[];
 ...
 * prevents nothing. See lib/split.ts for why the cut is by topic.
 */
function splitLine(
  results: readonly ScoredResult[],
  keyOf: (r: ScoredResult) => string,
...
function topicTable(label: string, rows: readonly MacroRow[], limit: number): string {
  const lines = [`${label}`];
...
// page content into the gold set instead; see judged/ and gold.ts.
function main(): void {
  const ranker = load(arg('--index') ?? undefined);
  const out: { generated: string; sets: { artificial?: ArtificialReport } } =
    { generated: arg('--stamp', ''), sets: {} };
  ...
  if (fs.existsSync(artificialFile)) {
    const data = JSON.parse(fs.readFileSync(artificialFile, 'utf8')) as
      { main: ArtificialCase[]; typos: ArtificialCase[] };
    const results = evaluate(ranker, data.main);
    const byBucket = macro(results, (r) => (r as ArtificialCase).bucket ?? '(none)');
    ...
    console.log(`  accuracy         ${(summarise(strict).accuracy ?? 0).toFixed(1)}%`);
    ...
    out.sets.artificial = {
      ...
      results: results.map((r) => ({
        q: r.query, p: r.position, l: (r as ArtificialCase).bucket,
      })),
    };
  }
  ...
  if (compare && fs.existsSync(compare)) {
    const before = JSON.parse(fs.readFileSync(compare, 'utf8')) as
      { sets?: { artificial?: ArtificialReport } };
    ...
    if (!b || !a) {
      ...
      // `accuracy` is optional on Summary because summarise() returns `{ total: 0 }`
      // for an empty set — which a stored run of zero queries would be.
      const wasAccuracy = b.summary.accuracy ?? 0;
      const nowAccuracy = a.summary.accuracy ?? 0;
      const delta = nowAccuracy - wasAccuracy;
      const macroDelta = a.macro - b.macro;

      console.log(
        `  artificial  micro ${wasAccuracy.toFixed(1)}% -> ${nowAccuracy.toFixed(1)}% ` +
      ...
      const positions = new Map<string, number>(b.results.map((r) => [r.q, r.p]));
      let improved = 0;
      let worsened = 0;
