// labels.js — the label set as DATA: how it is identified, how it is written, and what makes it
 ...
 */
function writeCollection(file, meta, arrays) {
  const indent = (value) => JSON.stringify(value, null, 1).split('\n').join('\n ');
 ...
 *
 * @returns {string[]} empty when the set is sound.
 */
function integrity(gold, quarantine) {
  const problems = [];
  const seen = new Map();
...
const bySortKey = (a, b) => String(a.query).localeCompare(String(b.query), 'en')
  || String(a.src).localeCompare(String(b.src), 'en');

module.exports = { fingerprint, writeCollection, integrity, bySortKey };
