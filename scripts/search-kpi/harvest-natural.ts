#!/usr/bin/env node
// harvest-natural.ts — collect REAL search strings people type, from Google autocomplete.
//
//   node scripts/search-kpi/harvest-natural.ts [--out FILE]
//
// WHY AUTOCOMPLETE
//
...
// check-kpi.ts asserts that judged/*.js decides every query in this file exactly once, so a query
...
const OUT: string = (() => {
  ...
  const given = i === -1 ? undefined : process.argv[i + 1];

  return given ?? path.join(import.meta.dirname, 'data', 'natural-queries.json');
})();
...
async function suggest(prefix: string): Promise<string[]> {
  ...
  try {
    ...
    return Array.isArray(body[1]) ? body[1] as string[] : [];
  } catch {
    return [];
  }
}
