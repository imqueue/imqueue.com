#!/usr/bin/env node
// build-gold.ts — assemble data/gold.json and data/quarantine.json from the judged/ decisions.
//
...
import {
  fingerprint, writeCollection, bySortKey, type Label, type Quarantined,
} from './lib/labels.ts';
import { QUESTION } from './judged/question.ts';
import { INTENT } from './judged/intent.ts';
import { PEER } from './judged/peer.ts';
import type { NegativeJudgement, PositiveJudgement } from './judged/types.ts';

const SP = import.meta.dirname;
const ROOT = path.join(import.meta.dirname, '..', '..');
const KPI = import.meta.dirname;
const DATA = path.join(KPI, 'data');

// MODULES below is a runtime list, so the loads it drives cannot be static
// imports. Synchronous, because the assembly it feeds is.
...
const MODULES = ['mcp.ts', 'mcp-spelled.ts', 'noise-molecule.ts', 'jobqueue.ts',
  'comparisons.ts', 'delivery-monolith-testing.ts', 'infra.ts', 'core.ts'];
...
const queries: Label[] = [];
const contentGap: Quarantined[] = [];
const negative: Quarantined[] = [];
const unclassified = new Set<string>();
const order = new Map(natural.map((q, i) => [q, i]));

for (const file of MODULES) {
  const mod = require(path.join(SP, 'judged', file)) as {
...
if (unclassified.size) {
  console.error('These quarantine verdicts are in neither ON_TOPIC nor OFF_TOPIC in build-gold.ts.');
