...
// PTES (the Penetration Testing Execution Standard) is the other backbone the
// report cross-references, one phase per finding.
...
export interface Sink {
  ...
  /** Record that a check ran and found nothing — feeds the coverage summary. */
...
export function toMarkdown(report: Report, meta: ReportMeta): string {
  ...
  L.push(`## 2. Methodology & scope (NIST SP 800-115)`);
  L.push("");
  L.push(
    `This assessment follows the four NIST SP 800-115 phases — Planning, Discovery, ` +
      `Attack, Reporting — cross-referenced to the PTES phases. It is fully automated ` +
      `and non-destructive: no data is modified, no mail is actually sent, and no ` +
      `availability test (load/DoS) is performed. Targets in scope: ${meta.targets.join(", ")}.`,
  );
  L.push("");
  ...
  const phases: NistPhase[] = ["Discovery", "Attack"];
  L.push(`## 4. Findings`);
  L.push("");
  if (!open.length) {
  ...
  for (const phase of phases) {
    ...
    L.push(`### 4.${phase === "Discovery" ? "1" : "2"} ${phase}-phase findings`);
    L.push("");
  ...
  L.push("");
  L.push(
    `${report.passed.size} check(s) executed and passed across ${meta.targets.length} target(s). ` +
