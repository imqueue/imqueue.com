#!/usr/bin/env node
// check-mermaid.ts — structural sanity for the mermaid diagrams in src/.
//
//   node scripts/check-mermaid.ts
//
...
/** One ```mermaid block found in a markdown file. */
interface MermaidBlock {
  /** 1-based line of the block's first content line. */
  line: number;
  src: string[];
  caption: string;
  /** Set only when the fence is never closed. */
...
function blocksIn(file: string): MermaidBlock[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i] ?? '';
    const open = /^(~~~|```)mermaid\s*$/.exec(line);

    if (!fence && open) { fence = open[1] ?? ''; buf = []; start = i + 1; continue; }
    if (fence && line.trimEnd() === fence) {
