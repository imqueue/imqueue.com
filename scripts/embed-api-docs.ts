...
// Run: node scripts/embed-api-docs.ts
import fs from 'node:fs';
import path from 'node:path';

const ROOT = process.cwd();
const LATEST: Record<string, string> = { core: '3.2.1', rpc: '3.2.1' };
...
/** One heading-level group of the generated sidebar. */
interface NavGroup {
  group: string;
  items: NavItem[];
}
...
for (const [pkg, version] of Object.entries(LATEST)) {
  ...
  const urlFor = (base: string): string => (base === 'index' || base === pkg)
    ? `/api/${pkg}/${version}/`
    : `/api/${pkg}/${version}/${base}/`;
  const rewriteLinks = (html: string): string => html.replace(
    /href="(?:\.\/)?([A-Za-z0-9._-]+)\.html(#[^"]*)?"/g,
    (_m: string, base: string, anchor?: string) => `href="${urlFor(base)}${anchor || ''}"`,
  );
  ...
  const fixHome = (html: string): string => html.replace(/href="[^"]*">Home<\/a>/g, 'href="/api/">Home</a>');
  ...
  for (const sec of sections) {
    ...
    if (items.length) apiNav.push({ group, items });
  }
  ...
  // The package page is the one file whose <main> must be there: apiNav was just
  // parsed out of it, so reaching here without a body means the html changed
  // shape under us and the index would be written empty.
  if (idxBody === null) {
    throw new Error(`No <main class="apidoc"> in ${pkg}.html — cannot build the index page`);
  }
  fs.writeFileSync(
  ...
  console.log(`${pkg}@${version}: embedded ${count} pages + index (${apiNav.reduce((n: number, g) => n + g.items.length, 0)} sidebar symbols) -> ${path.relative(ROOT, outDir)}`);
}
