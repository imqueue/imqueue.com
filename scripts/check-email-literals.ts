#!/usr/bin/env node
// check-email-literals.ts — no email-shaped literal inside an inline SVG.
//
//   node scripts/check-email-literals.ts
//
...
// is actually off — is scripts/check-geo-live.ts, which this cannot see.
//
...
// cannot reach — see scripts/check-jsonld.ts.
//
...
// publishes on purpose — see the Organization node in scripts/check-jsonld.ts.
...
function hits(file: string): Array<{ line: number; text: string }> {
