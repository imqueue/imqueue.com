...
//    server searches. scripts/check-search-index.ts exists so it cannot happen
//    a third time.
...
import { slugify } from "./md-slug.ts";
import { lemmaOf, report, type MissingLemma } from "./lemma.ts";
import { requireRanker } from "./ranker.ts";

/** One parsed markdown mirror. */
export interface Mirror {
  title: string;
  url: string;
  body: string;
  /** 0-based file line index of the body's first line. */
  bodyLine: number;
}

/** One indexable section of a mirror body. */
export interface Section {
  heading: string;
  level: number;
  anchor: string;
  /** [start, end) within the body, half-open, so end - start is the line count. */
  start: number;
  end: number;
  text: string;
  emphasis: string;
...
/** A section still being accumulated by splitSections(). */
...
/**
 * One tier-1 record, in the ranker's wire shape.
 *
 * The keys are single letters because this ships to every visitor who searches.
 * A tuple position or a top-level key changing is a FEED_V bump — see below.
 */
export interface FeedRecord {
  /** Group: 0 page, 1 API symbol, 2 FAQ answer. */
  g: number;
  /** Title. */
  t: string;
  /** URL. */
  ...
  /** Summary. */
  s: string;
  /** Badge label, or the symbol kind on an API record. */
  k: string;
  /** Curated keywords, only on a page that has them. */
  w?: string;
  /** Package name, only on an API record. */
  p?: string;
  /** 1 when the symbol is deprecated. */
  d?: number;
}

/** One entry of the generated /api/search-index.json feed. */
...
/** Curated per-URL front matter, from the build intermediate. */
...
/** Tier 1: everything the result list needs, fetched first. */
...
/** Tier 2: the prose, fetched behind tier 1. */
export interface Tier2 {
  v: number;
  e: number;
  /** [url, title, group] */
  pages: Array<[string, string, string]>;
  /** [pageIndex, anchor, heading, text, emphasis] */
  sections: Array<[number, string, string, string, string]>;
  /** Surface form -> lemma, delta only. */
  lemmas: Record<string, string>;
}

/** Anchor -> [startLine, endLine) into each page's markdown mirror. */
...
/** What the build produced, for the generator to report. */
export interface CorpusStats {
  ...
  keyworded: number;
  lemmas: number;
  missingWords: MissingLemma[];
}

/** Everything buildCorpus() produces. */
...
export function groupFor(url: string): string {
 ...
 */
export function plainText(md: unknown): string {
 ...
 *
 * @returns null when the file is not a mirror (no `# ` title, or no `Source:`
...
// submodule's path — that module exists to be the only place the path appears,
// and this line was one of the spellings its header counts. The visible change
// is the failure message: an unpopulated submodule now says how to populate it
// instead of raising MODULE_NOT_FOUND on a path nothing explains.
const ENGINE_V = requireRanker().ENGINE_V;

export function parseMirror(text: unknown): Mirror | null {
...
export function splitSections(body: string): Section[] {
  ...
  let current: OpenSection = { heading: "", level: 0, anchor: "", lines: [], start: 0 };
 ...
 */
export function headingText(md: unknown): string {
  return String(md == null ? "" : md)
    .replace(/!\[[^\]]*\]\([^)]*\)/g, " ")
    .replace(/\[([^\]]*)\]\([^)]*\)/g, "$1")
 ...
 */
export function emphasized(md: unknown): string {
  const found: string[] = [];
 ...
 */
export function faqRecords(sections: Section[], page: Pick<Mirror, "url" | "title">): FeedRecord[] {
  return sections
 ...
 *
 * @param outputDir e.g. "_site-org".
 */
// Ids present in a built page, read once per page.
const idCache = new Map<string, Set<string>>();

function pageIds(outputDir: string, url: string): Set<string> {
  // get() once and return it, rather than has()-then-get(): the Map's value type
  // says the second lookup could miss, and the cache is the whole point of the
  // function — a `!` here would be asserting away its only failure mode.
  const cached = idCache.get(url);
 ...
 */
function verifyAnchor(part: Section, outputDir: string, url: string): Section {
 ...
 */
function vocabularyOf(text: unknown, into: Set<string>): Set<string> {
...
function lemmaMap(words: Set<string>): Record<string, string> {
 ...
 * src/search-frontmatter.11ty.ts writes.
 *
 ...
 */
function takeFrontmatter(outputDir: string): Record<string, CuratedFrontmatter> {
...
export function buildCorpus(outputDir: string): Corpus {
  const docs: FeedRecord[] = [];
  const faq: FeedRecord[] = [];
  const pages: Tier2["pages"] = [];
  const sections: Tier2["sections"] = [];
  // url -> { anchor: [startLine, endLine) } into the markdown mirror. See the walk below.
  const ranges: SectionRanges["pages"] = {};
  ...
  const walk = (dir: string): void => {
    ...
    for (const entry of entries) {
      ...
      for (const part of parts) {
        ...
        if (part.anchor) {
          const forPage = ranges[mirror.url] ?? {};

          ranges[mirror.url] = forPage;
          forPage[part.anchor] = [
  ...
  if (fs.existsSync(apiFeed)) {
    const feed = JSON.parse(fs.readFileSync(apiFeed, "utf8")) as ApiFeedEntry[];

    api = feed.map((s) => {
      const entry: FeedRecord = {
        g: 1, t: s.name, u: s.url, s: s.summary || "", k: s.kind || "", p: s.package,
      };
