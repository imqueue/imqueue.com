...
//
// Shared by scripts/build-api-docs.ts (at generation time),
// scripts/backfill-api-descriptions.ts (which applies it to pages that were
// generated before this existed), and src/org/api/search-index.11ty.ts (which
...
export function summaryParagraph(md: string): string {
...
/** Which page a description is being built for. */
export interface ApiPageRef {
  /** Package name without the scope, e.g. "rpc". */
  pkg: string;
  /** Version the page documents. */
  version: string;
  /** Heading text, e.g. "IMQRPCError interface". */
  symbol: string;
}
 ...
 *
 * @param md Raw api-documenter markdown (front matter optional).
 * @returns A description that is unique to this page.
 */
export function apiDescription(md: string, { pkg, version, symbol }: ApiPageRef): string {
