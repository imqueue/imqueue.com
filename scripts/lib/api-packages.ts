// api-packages.ts — the single source of truth for which @imqueue packages get a
...
//
//   scripts/build-api-docs.ts   which packages to generate, and per-package policy
//   scripts/gen-api-crosslinks  which packages to diff for stripped re-exports
//   src/_data/apiPackages.ts    the /api/ landing page's Tier 2 group sections
...
//     `archive ? ... : /api/<pkg>/latest/` fallback in lib/api-redirects.ts:83.
...
/** A tag's only behaviour: whether it marks a pick-at-most-one set. */
export interface TagRule {
  exclusive: boolean;
}

/** One package the site generates an API reference for. */
export interface Package {
  /** Unscoped name, e.g. "pg-pubsub". */
  name: string;
  /** 1 for the framework spine, 2 for everything grouped on /api/. */
  tier: number;
  /** GROUP_ORDER section, or null for tier 1. */
  group: string | null;
  tags: string[];
  blurb: string;
  /** A trade-off a human reader needs stated, shown on /api/. */
  note?: string;
  /** The same trade-off as a RULE, for an agent choosing between packages. */
  agentNote?: string;
  /** The id `imq` knows it by, when the CLI can install it. */
  cliId: string | null;
  /** Whether older majors are swept rather than archived. */
  latestOnly: boolean;
  status: string;
  wave: number;
}

/** One package published to npm that the site does NOT generate docs for. */
export interface UndocumentedPackage {
  name: string;
  docs: string;
  blurb: string;
}

/** A slug that moved, and where it moved to. */
export interface PackageRename {
  from: string;
  to: string;
}

/** One row of /status/: every published package, documented or not. */
...
/** One rendered section of the /api/ landing page. */
...
export const TAGS: Record<string, TagRule> = {
...
export const PACKAGES: Package[] = [
  ...
  // order of the generated src/_data/apiVersions.json and lib/api-versions.ts.
...
(function validate(): void {
  ...
  for (const p of PACKAGES) {
    ...
    if (p.tier === 2 && (p.group === null || !GROUP_ORDER.includes(p.group))) {
 ...
 * lib/api-redirects.ts, which rewrites both halves.
 *
 * This is the source of truth for three things that have to agree:
 *
 *   * lib/api-renamed.ts       generated from here by build-api-docs.ts; the
 ...
 * build-api-docs.ts generate an /api/<name>/ tree and mount a Pages Function for a
 ...
 */
export const UNDOCUMENTED_PACKAGES: UndocumentedPackage[] = [
  {
    name: 'cli',
    docs: '/cli/',
 ...
 *
 */
export function statusPackages(): StatusPackage[] {
  return [
    ...shipped().map(p => ({
      name: p.name,
      blurb: p.blurb,
...
(function validateStatusPackages(): void {
 ...
 *
 */
export function shippedGroups(): PackageGroup[] {
  return GROUP_ORDER
    .map(group => ({
      group,
