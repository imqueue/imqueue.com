 ...
 *
 *   node scripts/gen-og-blog.ts      (or: npm run gen-og-blog)
 *
 ...
 *
 * Fonts are handled exactly like gen-og-images.ts (woff2 -> sfnt in a temp dir
...
/** One entry of src/org/_data/authors.yml. */
interface Author {
  slug: string;
  name: string;
}
...
function frontMatter(src: string): PostFrontMatter {
  const m = src.match(/^---\n([\s\S]*?)\n---/);
  return m ? (yaml.load(m[1] ?? '') ?? {}) as PostFrontMatter : {};
}
...
function buildSVG(
  { title, illu, byline }: { title: string; illu: string | null; byline: string },
): string {
...
async function main() {
  ...
  const authorsList = (yaml.load(fs.readFileSync(AUTHORS_FILE, "utf8")) ?? []) as Author[];
  const authors: Record<string, Author | undefined> =
    Object.fromEntries(authorsList.map((a) => [a.slug, a]));
  ...
  for (const file of files) {
    ...
    const author = fm.author ? authors[fm.author] : undefined;
