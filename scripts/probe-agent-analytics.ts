#!/usr/bin/env node
// probe-agent-analytics.ts — does the agent-analytics pipeline actually deliver?
//
...
//
// It calls the same lib/agent-analytics.ts the Cloudflare middleware calls, so a pass
...
// rule 1 in lib/agent-analytics.ts: both streams land in the SAME property on
...
function sitePropertyIds() {
  try {
    const cfg = fs.readFileSync(path.join(ROOT, 'eleventy.config.mts'), 'utf8');
...
// design.
/** The optional fourth element of a probe case. */
interface ProbeExtra {
  referrer?: string;
  document?: boolean;
}

/** One probe: user-agent, path, status the edge would have returned, and extras. */
type ProbeCase = readonly [string, string, number, ProbeExtra?];
...
async function main() {
  ...
  // Typed as the same open record the Pages runtime hands the middleware, so the
  // probe cannot accidentally exercise a narrower env than production has.
  ...
  if (!isSiteProperty && siteIds.size && !force) {
    ...
    console.error(
      ...
      + `  eleventy.config.mts names: ${[...siteIds].join(', ')}\n\n`
      + 'Both streams share one property by design (rule 1 in lib/agent-analytics.ts);\n'
  ...
  const { trackRequest, buildEvent } = await import('../lib/agent-analytics.ts');
  ...
  for (const [userAgent, pathname, status, extra = {}] of CASES) {
    ...
    const params = built?.events[0]?.params;

    // Every case above is one describe() counts, so a null here means the module
    // stopped counting something the probe exists to prove it still counts —
    // which is worth a failure rather than a crash on `.events` of null.
    if (!params) {
      console.error(`  FAIL  ${pathname} produced no event — buildEvent declined it`);
      process.exit(1);
    }

    await trackRequest({
      request: { headers: { get: (h: string) => headers[h.toLowerCase()] ?? null } },
      env: probeEnv,
      url,
      status,
      edition: 'org',
