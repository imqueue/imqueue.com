#!/usr/bin/env node
// check-search-ui.ts — asserts that search can actually be reached.
//
//   node scripts/check-search-ui.ts
//
// check-search-ranking.ts proves the ranker returns the right page. It cannot prove a
...
/** One assertion over a source file: the pattern, what it proves, what it means if absent. */
type SourceCase = readonly [re: RegExp, ok: string, why: string];
...
const jsFile = path.join(ROOT, 'src', '_shared', 'js', 'site.ts');
const js = read(jsFile);

const JS_CASES: readonly SourceCase[] = [
  [
    /aria-expanded['"]\s*,\s*String\(/,
    'site.ts writes aria-expanded when the drawer moves',
    'site.ts never sets aria-expanded — the burger shipped stuck at "false" while the drawer opened',
  ],
  [
    /\[data-mobile-nav\][^'"]*\[data-search-open\]/,
    'site.ts closes the drawer when its search trigger is tapped',
    'site.ts only closes the drawer on `a` taps, so opening search left the drawer sitting ' +
    'open behind the dialog and still open after it was dismissed',
  ],
  // Deliberately not "a resize listener exists": site.ts has another one for the TOC, so
  ...
  [
    ...
    'site.ts closes the drawer on resize, so it is not stranded past the breakpoint',
...
// check a constant that never leaves the IIFE. From the submodule — see scripts/lib/ranker.ts,
...
const SEARCH_CASES: readonly SourceCase[] = [
  [
...
const SCOPE_CASES: readonly SourceCase[] = [
