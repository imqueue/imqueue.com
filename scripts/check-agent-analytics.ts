#!/usr/bin/env node
// check-agent-analytics.ts — the five things about lib/agent-analytics.ts and
// functions/_middleware.ts that are worth a gate.
//
...
import type {
  BuildEventInput, Ga4Event, Ga4Params,
} from '../lib/agent-analytics.ts';
...
let checks = 0;
const ok = (msg: string): void => { checks++; console.log(`  ok    ${msg}`); };

async function main(): Promise<void> {
  const { buildEvent, trackRequest } = await import('../lib/agent-analytics.ts');
  const { onRequest } = await import('../functions/_middleware.ts');

  /**
   * buildEvent for the cases that MUST be counted.
   *
   * It returns null for a request it declines, and every call through here is one
   * the module is being asserted to count — so a null is this check failing, not a
   * branch to handle. One assertion in one place beats a `!` at eleven call sites,
   * and it names the path that stopped being counted. The three calls that expect
   * null go to buildEvent directly, which is what makes the difference visible.
   */
  const built = async (input: BuildEventInput): Promise<Ga4Event> => {
    const event = await buildEvent(input);

    assert.ok(event, `buildEvent declined ${input.url.pathname} — this case must be counted`);

    return event;
  };

  const page = new Response('hi', { status: 200 });
  const ctx = (url: string, extra: Partial<PagesContext> = {}): PagesContext => ({
  ...
  // A deliberately unparseable Request. Cast because a real `new Request()` will
  // not accept the malformed url this needs — refusing it is the browser API
  // behaving correctly, and the whole point here is what the middleware does when
  // `new URL()` throws inside the analytics block.
  const unparseable = await onRequest({
    request: { url: '://not-a-url', headers: { get: () => null } } as unknown as Request,
  ...
  // know that, and check-agent-analytics.ts records that a previous half-hour
  // assertion was deleted as ceremony — this one carries the reason so it is not
  // deleted again.
  const at = async (ms: number): Promise<string | number> => (await built({
  ...
  // a visible link — all three from hasMarkdownMirror() in lib/markdown-link.ts, so
  ...
  {
    const { hasMarkdownMirror, markdownLink } = await import('../lib/markdown-link.ts');

    const MIRRORS: ReadonlyArray<readonly [string, boolean, string]> = [
    ...
    for (const [path, want, why] of MIRRORS) {
  ...
  const paramsFor = async (
    userAgent: string, path: string, isDocument = false,
  ): Promise<Ga4Params> => (await built({
  ...
  // [user-agent, path, isDocument, expected kind]
  ...
  {
    const refFor = async (
      referrer: string | null, path = '/docs/index.md',
    ): Promise<Ga4Params> => (await built({
      url: new URL(`https://imqueue.org${path}`),
    ...
    // [referrer, expected referrer_host, expected ai_source]
    const cases: ReadonlyArray<readonly [string | null, string, string]> = [
  ...
  {
    const { ga4ClientId } = await import('../lib/agent-analytics.ts');
    ...
    const browserEvent = await built({
      url: new URL('https://imqueue.org/docs/'),
      userAgent: BROWSER, ip: '203.0.113.9', salt: 'test-salt', isDocument: true,
    ...
    const crawlerEvent = await built({
