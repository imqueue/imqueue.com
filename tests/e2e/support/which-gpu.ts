#!/usr/bin/env node
// which-gpu.ts — prove which GPU the test browser actually used.
//
//   node tests/e2e/support/which-gpu.ts            # headless (the suite's default)
//   node tests/e2e/support/which-gpu.ts --headed   # headed, pinned to the iGPU
//
// The configuration in playwright.config.ts CLAIMS the browser renders on the
...
import { gpuEnv, discreteStatus, describe } from './gpu';
...
async function main() {
  ...
  const renderer = await page.evaluate(() => {
    ...
    // `getContext` is overloaded by the string it is given, and only the literal
    // 'webgl' resolves to WebGLRenderingContext; 'experimental-webgl' falls to the
    // generic overload returning RenderingContext, which has no getParameter. The
    // cast names what the fallback actually hands back on the browsers that need it.
    const gl =
