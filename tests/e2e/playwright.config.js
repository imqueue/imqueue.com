...
// tests/e2e/server/pages-server.mjs, which runs the real Cloudflare Pages
...
// and no panic. tests/e2e/support/gpu.js has the full reasoning and does the work
...
const { gpuEnv, describe: describeGpu } = require('./support/gpu');
...
module.exports = defineConfig({
  ...
  use: {
    ...
    // stubbed in tests/e2e/support/fixtures.js; this is the second lock.
    bypassCSP: false,
  },
  projects: [
    {
  ...
  webServer: {
    command: `node ${__dirname}/server/pages-server.mjs --port ${PORT}`,
    url: `${BASE_URL}/`,
