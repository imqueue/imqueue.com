// nav.mobile.spec.ts — the phone. Runs on the `mobile` project (a real device
...
const drawer = (page: Page) => page.locator('[data-mobile-nav]');
const burger = (page: Page) => page.locator('[data-nav-toggle]');
...
test('the drawer carries a search entry point that has a size', async ({ page }) => {
  ...
  expect(box, 'the search entry point is laid out').not.toBeNull();
