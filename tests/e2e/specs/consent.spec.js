// consent.spec.js — the cookie banner, and the promise it makes.
//
...
const { test, expect, CONSENT_KEY } = require('../support/fixtures');

test.use({ consent: null });

const banner = (page) => page.locator('[data-consent-banner]');
const stored = (page) => page.evaluate((key) => window.localStorage.getItem(key), CONSENT_KEY);
...
test.describe('declining', () => {
  test('hides the banner, records the refusal and contacts nobody', async ({ page, external }) => {
    ...
    expect(JSON.parse(await stored(page))).toEqual({ analytics: false, replay: false });
    expect(external.googleAnalytics).toEqual([]);
    expect(external.clarity).toEqual([]);
  });
...
test.describe('allowing', () => {
  test('allow all loads both vendors, immediately and without a reload', async ({ page, external }) => {
    ...
    await expect(banner(page)).toBeHidden();
    expect(JSON.parse(await stored(page))).toEqual({ analytics: true, replay: true });
...
test.describe('choosing one purpose and not the other', () => {
  ...
  test('analytics on, session replay off — and only Google is contacted', async ({ page, external }) => {
    ...
    await expect(banner(page)).toBeHidden();
    expect(JSON.parse(await stored(page))).toEqual({ analytics: true, replay: false });
