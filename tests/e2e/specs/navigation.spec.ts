// navigation.spec.ts — getting around: the bar, the breadcrumbs, the footer, and
...
// scripts/check-links.ts, so this file does not re-crawl it. What it covers is
...
import { goto, internalLinks } from '../support/site';

test.describe('the nav bar', () => {
  test('the three section links go where they say', async ({ page }) => {
    await page.goto('/');

    // `as const` so the pairs stay pairs: without it this is a `string[][]`, each
    // element of which is `string | undefined`, and `toHaveAttribute` takes the
    // undefined arm for its options-object overload.
    const links = [['intro', '/intro/'], ['docs', '/docs/'], ['blog', '/blog/']] as const;
...
test.describe('the footer', () => {
  test('carries the legal pages, and they are real', async ({ page }) => {
    ...
    const response = await goto(page, '/terms/');
...
test.describe('dead ends', () => {
  test('an unknown URL gets the 404 page with a way out', async ({ page }) => {
    const response = await goto(page, '/does/not/exist/');
  ...
  test('an unknown /api/ package 404s rather than inventing a version', async ({ page }) => {
    ...
    const response = await goto(page, '/api/not-a-package/');
