// agent-surfaces.spec.ts — everything the site publishes for machines rather
...
//
// Two of the assertions are about HEADERS produced by functions/_middleware.ts,
...
test.describe('llms.txt', () => {
  ...
  test('every page it lists is real', async ({ request }) => {
    ...
    const paths = [
      ...new Set(
        Array.from(body.matchAll(/https:\/\/imqueue\.org(\/[^\s)]*)/g))
          .map((m) => m[1])
...
test.describe('the edge analytics diagnostic', () => {
  ...
  test('an ordinary asset is not measured here at all', async ({ request }) => {
    ...
    const stylesheet = /href="(\/css\/[^"]+\.css)"/.exec(html)?.[1];

    expect(stylesheet, 'a stylesheet link on the homepage').toBeTruthy();

    // Narrowed rather than asserted: the `expect` above fails the test on a miss,
    // but it is a runtime check the compiler cannot see, and the read below has
    // to be safe on its own terms.
    if (!stylesheet) {
      return;
    }
