// api-reference.spec.js — /api/, the generated reference, and the redirect policy
// that keeps its old URLs alive.
//
// Those redirects are the reason tests/e2e/server/pages-server.mjs imports the
// real Cloudflare Pages Functions instead of just serving files: on the deployed
// site /api/core/2.0.1/ is resolved by lib/api-redirects.js at request time, not
...
const hop = (request, url) => request.get(url, { maxRedirects: 0 });
...
test.describe('trimming a URL down', () => {
  ...
  test('/api/contact is not treated as a package', async ({ request }) => {
    ...
    // is why the Functions are mounted one level deeper — see lib/api-handler.js.
...
test('no redirect lands anywhere but a 200', async ({ request }) => {
  ...
  for (const url of urls) {
    ...
    const landed = await request.get(target(first), { maxRedirects: 0 });
...
test.describe('the reference pages themselves', () => {
  ...
  test('a symbol page carries its breadcrumbs and its signature', async ({ page }) => {
    const response = await page.goto('/api/core/latest/core.redisqueue/');
