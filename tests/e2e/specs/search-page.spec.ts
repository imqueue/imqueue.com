// search-page.spec.ts — /search/, the full result list.
//
...
import { attr, responseOf, settle, withoutHash } from '../support/site';

const results = (page: Page) => page.locator('[data-search-page] .s-hit');
...
test('a result leads to the page it names', async ({ page }) => {
  ...
  const [navigation] = await Promise.all([page.waitForNavigation(), first.click()]);
