// pages.spec.js — every part of the site loads, and loads whole.
//
...
const { KEY_PAGES } = require('../support/site');
...
test('the 404 page is served, with its own status and the site chrome', async ({ page }) => {
  const response = await page.goto('/this-page-was-never-published/');
...
test('a directory URL without its trailing slash is redirected, not served', async ({ page }) => {
  ...
  const response = await page.goto('/cli/installation');
