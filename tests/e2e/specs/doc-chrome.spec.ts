// doc-chrome.spec.ts — the behaviours a documentation page has that a page does
...
// size.
//
// All four are in src/_shared/js/site.ts, all four are invisible to every check
...
test.describe('the table of contents', () => {
  ...
  test('clicking an entry moves to that section', async ({ page }) => {
    ...
    const href = await attr(entry, 'href');
...
test.describe('deep links into an accordion', () => {
  ...
  test('navigating to another question opens that one too', async ({ page }) => {
    ...
    const slug = await attr(page.locator('.faq details').nth(2), 'id');
