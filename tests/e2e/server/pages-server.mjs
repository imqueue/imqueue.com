// pages-server.mjs — a local stand-in for the Cloudflare Pages deployment.
//
//   node tests/e2e/server/pages-server.mjs [--dir _site-org] [--port 8099]
//
...
//
//   1. functions/_middleware.js — the parked-domain 301, the `x-agent-analytics`
//      diagnostic header and the `Link: …index.md; rel="alternate"` mirror hint.
//   2. functions/api/<pkg>/[[path]].js — every retired API version URL, every
//      renamed package slug and every TypeDoc-era deep link resolves HERE, not in
//      _redirects (see lib/api-redirects.js for why).
...
const inside = (p) => p === SITE || p.startsWith(SITE + path.sep);
...
const body = async (file) => new Uint8Array(await readFile(file));
...
const { handleApiRequest } = await import(path.join(ROOT, "lib", "api-handler.js"));
...
function apiPackageOf(pathname) {
  const m = /^\/api\/([^/]+)(?:\/|$)/.exec(pathname);

  return m && API_PACKAGES.has(m[1]) ? m[1] : null;
}
...
const server = createServer(async (req, res) => {
  try {
    ...
    res.end(`pages-server: ${error && error.stack}`);
  }
});
