// markdown-link.js — where a page's plain-markdown mirror is, and who is told.
//
// Three consumers, ONE rule:
//
//   * src/{org,com}/*.11tydata.js  -> `hasMirror` computed data, which head.html
//     turns into <link rel="alternate" type="text/markdown"> and
//     mirror-link.html turns into the visible "read as markdown" link.
//   * functions/_middleware.js     -> the `Link: …; rel="alternate"` header.
//   * scripts/check-agent-analytics.js -> asserts the cases below.
//
...
//
// The rule itself is the NEGATION of check-sitemap.js's mirror-coverage
...
// /cli/installation/; 79,626 vs 18,068 on /tutorial/user-service/.
...
const NO_MIRROR = [
  ...
  /\/page\/\d+\/$/,
  // Archived API majors are noindex and unmirrored; only /api/ itself and the
  // /latest/ trees have mirrors. Reads as "under /api/, not /api/ itself, and not
  // inside a /latest/ tree".
  /^\/api\/(?!$)(?!.*\/latest\/)/,
];
 ...
 *
 * @param {string} pathname  root-relative, directory-shaped ("/cli/installation/")
 * @param {{draft?: boolean, mirror?: boolean}} [flags]
 *   Front matter. `mirror: false` is the per-page opt-out. `draft` excludes,
 *   because the contentMd collection in eleventy.config.js skips drafts, so no
 *   mirror is emitted for them.
 *
 *   `noindex` deliberately does NOT exclude. check-sitemap.js does not *require* a
 ...
 * @returns {boolean}
 */
function hasMarkdownMirror(pathname, flags = {}) {
 ...
 *
 * @param {{url: URL, status: number, response: {headers: {get: (n: string) => string|null}}}} args
 * @returns {string|null}
 */
function markdownLink({ url, status, response }) {
...
export { hasMarkdownMirror, markdownLink };
