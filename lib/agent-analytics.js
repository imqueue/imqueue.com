...
/**
 * One line describing what this module WOULD do with a request, for the
 * `x-agent-analytics` response header the middleware adds when GA4_MP_DEBUG is set.
 *
 * Exists because "is the site sending events?" is otherwise only answerable by
 * touring dashboards: GA4's own reports cannot distinguish "never sent" from "sent
 * and rejected" from "sent to a property you are not looking at". This makes the
 * server-side half a single curl.
 *
 * Deliberately reuses buildEvent and the same credential guard as trackRequest
 * rather than restating the policy — if that guard changes, change it here too.
 */
export function debugNote({ request, env, url, status, edition }) {
  ...
  return `sent crawler=${p.crawler} surface=${p.surface} status=${p.status} edition=${p.edition}`;
}
