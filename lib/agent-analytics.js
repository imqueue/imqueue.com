 ...
 *
 * @param {string|null} referrer The Referer header, verbatim.
 * @param {URL} url The request URL, to recognise same-origin.
 * @returns {{referrer_host: string, ai_source: string}}
 */
export function classifyReferrer(referrer, url) {
  if (!referrer) {
 ...
 * @param {boolean} [botish] UA declares itself a bot but matches no known family.
 * @returns {string} The audience bucket.
 */
export function classifyKind(found, surface, browser = false, botish = false) {
  if (found && found.kind) {
    return found.kind;
  }
