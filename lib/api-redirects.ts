...
// actually publishes.
//
// Policy (identical to the one scripts/build-api-docs.ts builds pages for):
...
//
// Pure and dependency-free so scripts/check-redirects.ts can exercise it under
// plain node against every published version.

import { API_VERSIONS } from "./api-versions.ts";
import type { ApiVersions } from "./api-types.ts";
import { ARCHIVED_PAGES } from "./api-legacy-pages.ts";
import { CORE_REEXPORTS } from "./api-crosslinks.ts";
import { RENAMED_API_PAGES } from "./api-renames.ts";
import { RENAMED_PACKAGES } from "./api-renamed.ts";

const SEMVER = /^\d+\.\d+\.\d+$/;
const majorOf = (v: string): number => Number(v.split(".")[0]);
 ...
 * @param [versions] Version map; defaults to the generated one.
 * @returns Pathname to 301 to, or null if this is not a retired slug.
 */
export function resolveRenamedPackage(
  pathname: string,
  versions: ApiVersions = API_VERSIONS,
): string | null {
 ...
 *
 * @param pathname Request pathname, e.g. "/api/rpc/3.5.1/rpc.imq/".
 * @param [versions] Version map; defaults to the generated one.
 * @returns Pathname to 301 to, or null to serve the path as-is.
 */
export function resolveApiRedirect(
  pathname: string,
  versions: ApiVersions = API_VERSIONS,
): string | null {
  ...
  if (root) {
    const rootPkg = root[1] ?? "";

    return versions[rootPkg] ? `/api/${rootPkg}/latest/` : null;
  }
  ...
  // source of `index.md`, byte-identical. build-api-docs.ts now skips the former,
  ...
  // A past major keeps its highest release; build-api-docs.ts archives every
  ...
  const archive = plan.archives.find((a: string) => majorOf(a) === majorOf(seg));
 ...
 * the version index rather than guessing a page.
 *
 * A symbol whose page is NOT in that tree also lands on the version index. That case
 * is real and was shipping 301-into-404 until this check existed:
 * /api/rpc/2.1.0/interfaces/IMQOptions.html mapped to /api/rpc/2.1.0/rpc.imqoptions/,
 * which rpc never documented — IMQOptions is core's. Guessing a symbol page is only
 * safe when the page is known to exist, so ARCHIVED_PAGES (generated from the tree)
 * decides, and the index is the floor. A 301 into a 404 is worse than the 404.
 *
 * @param pathname Request pathname.
 * @returns The page to 301 to, or null if there is no match.
 */
export function resolveLegacyTypedoc(pathname: string): string | null {
  ...
  const page = `${pkg}.${name.toLowerCase()}`;

  return ARCHIVED_PAGES.has(`${pkg}/${seg}/${page}`)
    ? `/api/${pkg}/${seg}/${page}/`
    : `/api/${pkg}/${seg}/`;
}
 ...
 *
 * @param pathname Request pathname.
 * @returns The core page to 301 to, or null if there is no match.
 */
export function resolveCoreReexport(pathname: string): string | null {
  const m = /^\/api\/rpc\/latest\/rpc\.([A-Za-z0-9._-]+?)\/?$/.exec(pathname);

  const symbol = m?.[1]?.toLowerCase();

  if (!symbol || !CORE_REEXPORTS.has(symbol)) {
    return null;
  }

  return `/api/core/latest/core.${symbol}/`;
}
 ...
 * @returns The page to 301 to, or null if there is no match.
 */
export function resolveRenamedApiPage(pathname: string): string | null {
  const m = /^\/api\/(.+?)\/?$/.exec(pathname);
