// visitor-id.js — a stable, non-reversible visitor identifier derived at the edge.
//
...
// A trend line, not a headcount; ±10–20% is normal and cannot be tuned away.
 ...
 *
 * @param {object} args
 * @param {string} args.ip          Client address (Cloudflare's cf-connecting-ip).
 * @param {string} [args.userAgent] Client user-agent; separates visitors behind one NAT.
 * @param {string} args.salt        Secret from VISITOR_SALT. Without it, null.
 * @returns {Promise<string|null>} 32 hex characters, or null.
 */
export async function visitorId({ ip, userAgent, salt }) {
  if (!salt || !ip) {
    return null;
  }
